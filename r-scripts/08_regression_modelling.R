...
orchdat<-readr::read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vT6bdROUwchBBIreCQQdBenu35D5Heo8bl3UgES9wrpBwax_GUM1bKSo_QXfmLq8Ml9-crCI7MmW2XH/pub?gid=615066255&single=true&output=csv") |> dplyr::filter(year %in% c(2018,2019,2021,2022,2023,2024),replicate<=3,Species_ID=="Orchestia_gammarellus") |>
...
orchat3 |>
  ...
  facet_grind(year~.) # what is in rows ~what is in columns
...
p1<- orchdat3  |> 
  ggplot(mapping=aes(x=elevation, y=CountSum, col=year) +
  geom_point(size=3)
